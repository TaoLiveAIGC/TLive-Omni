...
from vllm.model_executor.models.tlive_omni_configuration import SUPPORTED_TRANSFORMERS_VERSION
...
def _check_if_video_has_audio(video_path):
    import av
    with av.open(_normalize_media_path(video_path)) as container:
...
class TLiveOmniProcessor(ProcessorMixin):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """
        Processes text, image, video, and audio inputs for TLive-Omni.

        Args:
            text (`str`, `list[str]`, optional): Text prompt or batch of prompts.
            images (`PIL.Image.Image`, `np.ndarray`, `torch.Tensor`, `list`, optional): Image input or batch of image inputs.
            videos (`np.ndarray`, `torch.Tensor`, `str`, `list`, optional): Video input or batch of video inputs.
            audio (`np.ndarray`, `list`, optional): Audio waveform input or batch of audio waveforms sampled at 16 kHz.
            kwargs (`dict`, optional): Additional tokenizer, image, video, and audio processing arguments. TLive video options can be passed through `videos_kwargs`.

        Returns:
            [`~transformers.BatchFeature`]: Processed model inputs.
        """
        if text is None:
    ...
    def _calculate_timestamps(self, indices: Union[list[int], np.ndarray], video_fps: float, merge_size: int = 2, return_in_seconds: bool = False):
        ...
        if video_fps is None or video_fps <= 0:
            raise ValueError(f"Invalid video fps {video_fps!r} in metadata; cannot compute timestamps.")
