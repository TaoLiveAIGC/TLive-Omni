...
class Qwen2_5_VisionAttention(nn.Module):
    def __init__(
        ...
        # Use native (PyTorch) path for RoPE to match HF's float32 computation
        self.apply_rotary_emb = ApplyRotaryEmb(enforce_enable=False, enable_fp32_compute=True)
    ...
    def forward(
        ...
        if rotary_pos_emb_cos is not None and rotary_pos_emb_sin is not None:
            ...
            # Inline float32 RoPE to match HF's apply_rotary_pos_emb_vision
            _orig_dtype = qk_reshaped.dtype
            _qk_fp32 = qk_reshaped.float()
            _cos = rotary_pos_emb_cos.unsqueeze(-2).float()
            _sin = rotary_pos_emb_sin.unsqueeze(-2).float()
            _x1, _x2 = _qk_fp32.chunk(2, dim=-1)
            _o1 = _x1 * _cos - _x2 * _sin
            _o2 = _x2 * _cos + _x1 * _sin
            qk_rotated = torch.cat((_o1, _o2), dim=-1).to(_orig_dtype)
